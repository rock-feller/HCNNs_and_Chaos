...
import pytest
from torch.nn import MSELoss
# from models import LSpa_Model  # adjust import to your project structure
...
# @pytest.fixture
# def model(config):
#     return LSpa_Model(n_obs_vars= 3,
#         n_hid_vars = 5,
#         s0_nature = "random_",
#         train_s0= True,
#         batch_size =  2,
...
def input_data():
    ...
    n_obs_vars = model.n_obs_vars
    seq_len = 4
    return torch.randn(batch_size, seq_len, n_obs_vars)
...
def test_forward_shapes():
    ...
    n_obs_vars = model.n_obs_vars
    seq_len = 4
    input_data =  torch.randn(batch_size, seq_len, n_obs_vars)
    output = model(input_data, forecast_horizon=2)
    assert output.expectations.shape == (model.batch_size, input_data.shape[1], model.n_obs_vars)
    assert output.states.shape == (model.batch_size, input_data.shape[1], model.cell.n_state_vars)
    assert output.delta_terms.shape == (model.batch_size, input_data.shape[1], model.n_obs_vars)
    assert output.forecasts.shape == (model.batch_size, 2, model.n_obs_vars)
    assert output.future_states.shape == (model.batch_size, 2, model.cell.n_state_vars)

def test_forward_no_forecast():
    model = LSpa_Model(n_obs_vars= 3,
        n_hid_vars = 5,
        s0_nature = "random_",
        train_s0= True,
    ...
    seq_len = 4
    input_data =  torch.randn(batch_size, seq_len, n_obs_vars)
    output = model(input_data, forecast_horizon=None)
    assert output.forecasts is None
    assert output.future_states is None

def test_trainable_initial_state():
    model = LSpa_Model(n_obs_vars= 3,
        n_hid_vars = 5,
        s0_nature = "random_",
        train_s0= True,
        ...
        sparsity_ratio = 0.3,
        bias = False)
    assert model.s0.requires_grad is model.train_s0
    assert model.s0.shape == (1, model.cell.n_state_vars)
...
def test_model_backward_pass():
    ...
    input_data =  torch.randn(batch_size, seq_len, n_obs_vars)
    criterion = MSELoss()
    output = model(input_data, forecast_horizon=10)
    loss = criterion(output.expectations, input_data)
    loss.backward()
    grads = [p.grad for p in model.parameters() if p.grad is not None]
    assert all(g is not None and torch.any(g != 0) for g in grads), "Gradients did not flow through the model"

# def test_model_name_generation(model):
#     name = model._generate_model_name()
#     assert isinstance(name, str)
#     assert f"obs{model.n_obs_vars}" in name
#     assert f"hid{model.n_hid_vars}" in name
