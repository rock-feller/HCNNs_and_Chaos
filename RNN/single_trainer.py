...
#     def __init__(self, model: RNN_Model, optimizer: torch.optim.Optimizer,
#                  loss_fn: nn.Module, device: Optional[torch.device] = None,
#                  forecast_steps: int = 10, forecast_context: Optional[torch.Tensor] = None,
#                  calibration_externals: Optional[torch.Tensor] = None,
#                  forecast_externals: Optional[torch.Tensor] = None):

class RNNTrainer:
    def __init__(self, model: nn.Module , optimizer: torch.optim.Optimizer, 
                 loss_fn: nn.Module, num_epochs: int = 10,
        ...
        self.variable_names = [f"var_{i+1}" for i in range(model.output_size)] if variable_names is None else variable_names
    ...
    def train_only(self, data_loader)-> Tuple[list, list]:
        """
    ...
    def train_validate(self, data_loader, calibration_window:torch.Tensor, 
                       val_data: torch.Tensor)-> Tuple[list, list]:
        """
        Trains the model and returns logs of average losses and forecasts.
