...
class EnsembleLSTMTrainer:
            ...
            if backprop_mode == 'per_batch':
                ...
                for epoch in range(num_epochs):
                    ...
                    epoch_loss_summary[f'member_{idx+1}'] = member_loss_summary
                        ...
                        # epoch_losses.append({ f'member_{idx+1}': total_loss})
    ...
    def train_forecast(self, data_loader: DataLoader,
        ...
        #     self._save_losses_json(self.ensembles[f'member_{idx+1}'], losses, idx)

        return all_losses

    # def _save_forecasts_to_csv(self, out_seq: torch.Tensor, forecast: torch.Tensor, model_idx: int, epoch: int):
    #     all_data = []
    #     out_np = out_seq.cpu().numpy()
    #     fcast_np = forecast.cpu().numpy()

    #     for t, val in enumerate(out_np):
    #         all_data.append({"type": "context_output", "timestep": t, **{f"dim_{i}": v for i, v in enumerate(val)}})
    #     for t, val in enumerate(fcast_np):
    #         all_data.append({"type": "forecast", "timestep": t, **{f"dim_{i}": v for i, v in enumerate(val)}})

    #     df = pd.DataFrame(all_data)
    #     for i, var_name in enumerate(self.variable_names):
    #         df.rename(columns={f"dim_{i}": var_name}, inplace=True)

    #     fname = f"forecast_model{model_idx+1}_epoch{epoch+1}.csv"
    #     df.to_csv(os.path.join(self.save_dir, fname), index=False)
    #     print(f"📄 Forecast saved: {fname}")
    ...
    def load_ensemble_checkpoints(self, checkpoint_paths: str , train_type :str) -> List[dict]:
        ...
        if train_type == "train_only":
            ...
            # loaded_models = []
            # new_ensembles = RNNEnsemble(n_ensemble=len(self.ensembles), input_size=self.ensembles[0]['member_1'].input_size, 
            #                     hidden_size=self.ensembles[0]['member_1'].hidden_size, output_size=self.ensembles[0]['member_1'].output_size,
            #                 s0_nature=self.ensembles[0]['member_1'].s0_nature, train_s0=self.ensembles[0]['member_1'].train_s0,
            #                 init_range=self.ensembles[0]['member_1'].init_range, ext_vars=self.ensembles[0]['member_1'].ext_vars,
            #                     n_layers=self.ensembles[0]['member_1'].num_layers , optimizer=self.optimizer, learning_rate=self.ensembles[0]['optimizer_1'].param_groups[0]['lr'])
            ...
            for idx, checkpoint_path in enumerate(sorted(os.listdir(checkpoint_paths))):
                ...
                # model = RNN_Model(input_size=self.ensembles[0]['member_1'].input_size,
                #                 hidden_size=self.ensembles[0]['member_1'].hidden_size,
                #                 output_size=self.ensembles[0]['member_1'].output_size,
                #                 s0_nature=self.ensembles[0]['member_1'].s0_nature,
                #                 train_s0=self.ensembles[0]['member_1'].train_s0,
                #                 init_range=self.ensembles[0]['member_1'].init_range,
                #                 num_layers=self.ensembles[0]['member_1'].num_layers,
                #                 ext_vars=self.ensembles[0]['member_1'].ext_vars)

                # Create optimizer
                # if self.ensembles[0]['member_1'].optimizer_type == 'adam':
                #     optimizer = torch.optim.Adam(model.parameters(), lr=self.learning_rate)
                # elif self.ensembles[0]['member_1'].optimizer_type == 'sgd':
